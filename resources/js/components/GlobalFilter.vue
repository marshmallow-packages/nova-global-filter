<template>
  <div class="h-full md:col-span-12 col-span-12 w-full relative" style="min-height: 2rem">
    <div v-if="card.resettable" class="z-10 absolute top-0 mt-2 right-0 mr-2">
      <Link @click="resetFilters(card.filters)" class="text-xs font-semibold mr-2">
        {{ __('Reset') }}
      </Link>
    </div>
    <LoadingCard :loading="loading" class="md:col-span-12 h-full" v-if="card.filters.length > 0">
      <ScrollWrap class="items-start h-full grid md:grid-cols-12 gap-6 py-4 px-6">
        <div
        ...
        >
          <div class="w-full">
            ...
            <div v-if="filter.component === 'boolean-filter'" class="space-y-2 mt-2">
              <CheckboxWithLabel
  ...
  created() {
    Nova.$on('global-filter-request', filterClasses => {
      ...
      if (filterClasses && filterClasses.length) {
        filters = filters.filter(filter => filterClasses.includes(filter.class));
      }
